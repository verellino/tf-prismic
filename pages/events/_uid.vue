...
export default {
  async asyncData ({ $prismic, store, params }) {
    const article = await $prismic.api.getByUID('events', params.uid)
  ...
  computed: {
    formattedDate () {
      const date = this.$prismic.asDate(this.article.data.eventDate || this.article.first_publication_date)
