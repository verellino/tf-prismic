...
export default {
  ...
  async asyncData({ $prismic, store, params }) {
    const article = await $prismic.api.getByUID("report", params.uid);

    const { results: latestArticles } = await $prismic.api.query(
      $prismic.predicate.at("document.type", "report"),
      {
        orderings: `[${[
          { field: "my.report.publishDate", direction: "desc" },
    ...
    const { results: categoryPosts } = await $prismic.api.query([
      $prismic.predicates.at("my.report.categories", categoryId),
    ]);
